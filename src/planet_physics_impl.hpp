        ...
        :caster(c),world(w)
{
    ...
    const auto ent = world->player_entity =
    ...
    world->registry.assign<Player>(ent);
    world->registry.assign<Camera>(ent, Camera{2.0f, 0.0f});
    //Camera & cam = world->registry.get<Camera>(ent);
    //cam.distance = 2.0f;
    //cam.x = 0.0f;
...
void PhysicsPlanet<TCaster>::update(int dt) {
    ...
    Player &pcontrol = world->registry.get<Player>(pent);
    ...
    auto view = world->registry.view<Position, Velocity, Colour, Health>();
    ...
    if (!world->registry.valid(world->player_entity) || world->registry.empty())
    {
